...
# Count Vectorizer the tweets to train the model.
...
def featureExtraction(tweets, stopwordskl):
    ...
    #     lowercase=False, max_df=0.05, strip_accents='ascii', ngram_range=(1, 5))
    countvector = CountVectorizer(ngram_range=(1, 5))
    featurestrain = countvector.fit_transform(tweets)
    featureExtractionVector = countvector
    ...
    for modelname in modelname:
        ...
        for train_index, test_index in kf.split(X, y):
            ...
            if modelname != 'SVM':
                ...
                roc_auc = auc(fpr, tpr)
            # plot roc curves
                plotroccurves(modelname, y_pred_prob, fpr, tpr, roc_auc, predict)
        ...
        # print(f"The scores for the model {modelname} is ", accuracyscores)
        print(
...
if __name__ == "__main__":
    ...
    for i in l :
        ...
        # dataset['bigrams'] = dataset['tweets'].map(lambda x: list(find_bigrams(x.split(" "))))
        # print([list(item) for item in dataset['bigrams']])
        # dataset['bigrams'] = dataset['tweets'].map(lambda x: list(find_ngrams(x.split(" "), 2)))
        # print(dataset.bigrams.head())
        # exit(0)

        # normalize_corpus = np.vectorize(preprocessdata)
        # norm_corpus = normalize_corpus(dataset.tweets)
        # # print(norm_corpus)
        # print('The length :',len(norm_corpus))
        ...
        # # # k = pd.DataFrame(cv_matrix, columns=vocab)
        # exit(0)

        #  def generate_ngrams(tweets):
        #     # Convert to lowercases
        #     # Break sentence in the token, remove empty tokens
        #     tokens = tweets.apply(nltk.word_tokenize)
        #     # print(tokens)
        #     # exit(0)
        #     tweets = tweets.apply(ngrams(tokens, 5))
        #     return tweets
    '''
